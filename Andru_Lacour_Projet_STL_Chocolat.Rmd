...
```{r, message=FALSE, warning=FALSE}
plot(arima112$residuals, xaxt = "n", xlab ="Temps (en mois)", ylab = "Valeur des résidus", main = "Résidus du modèle ARIMA(1,1,2)")
library(car)
qqPlot(arima112$residuals, xlab ="Quantiles de la loi normale", ylab = "Quantiles de l'échatillon des résidus")
```
...
```{r, message=FALSE, warning=FALSE}
ggplot(ALL, aes(Temps, value)) + geom_line(aes(colour = series)) + geom_errorbar(aes(ymin = inf, ymax = sup), width = 0.5, color = "blue")
```
...
ggplot(ALL_red, aes(Temps, value)) + geom_line(aes(colour = series)) + geom_errorbar(aes(ymin = inf, ymax = sup), width = 0.7, color = "blue")
```
